...
async function run() {
  try {
    ...
    const coffeeCollection = client.db('chocolateDB').collection('chocolate')
    ...
    app.post('/coffees',async(req,res)=>{
        ...
        const result = await coffeeCollection.insertOne(chocolate)
        res.send(result)
    })
    app.get('/updateCoffees/:id', async(req,res)=>{
...
app.get('/',(req,res)=>{
    res.send("Chocolate server side are working !!")
})

app.listen(port, ()=>{
    console.log(`this server run is port number is ${port}`);
})
